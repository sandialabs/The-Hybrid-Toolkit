...
class gaussiannb_worker(modeling_worker):
  def __init__(self,**kwargs):
      # Call super class init first
      if (kwargs.get("default_name") == None):
          kwargs["default_name"] = "gaussiannb_worker"
      if (kwargs.get("model_type") == None):
          kwargs["model_type"]=sklearn_utils.gaussian_nb_model
  ...
  def loadFromJSON(json_data):
      ...
      if not (jsontype == "hybrid_sklearn.gaussiannb_worker.gaussiannb_worker"):
          return None

      return gaussiannb_worker(**json_data)
  ...
  def update_model(self):
    ...
    model_data = hybrid.data_blob.create ("gnb_model_data")
  ...
  def process_observations_core(self, observations, **kwargs):
    ...
    model_data = hybrid.data_blob.create ("gnb_eval_data")
    model_data.setMetaData ("observation_vectors", feature_vectors)
    model.setModelData (model_data)

    model.project_and_store (model_data, None)
    results = model.getMetaData ("gnb_results")

    for observation_index in range (len (observations)):
      self.setMetaData (observations[observation_index], "gnb_class_index", results[observation_index])
