...
class multinomialnb_worker(modeling_worker):
  ...
  def loadFromJSON(json_data):
      ...
      if not (jsontype == "hybrid_sklearn.multinomialnb_worker.multinomialnb_worker"):
          return None

      return multinomialnb_worker(**json_data)
  ...
  def update_model(self):
    ...
    model_data = hybrid.data_blob.create ("mnb_model_data")
  ...
  def process_observations_core(self, observations, **kwargs):
    ...
    model_data = hybrid.data_blob.create ("mnb_eval_data")
    model_data.setMetaData ("observation_vectors", feature_vectors)

    model.setModelData (model_data)
    model.project_and_store (model_data, None)

    results = model.getMetaData ("mnb_results")

    for observation_index in range (len (observations)):
      self.setMetaData (observations[observation_index], "mnb_class_index", results[observation_index])
