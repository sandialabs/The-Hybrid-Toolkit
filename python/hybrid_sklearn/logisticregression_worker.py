...
class logisticregression_worker(modeling_worker):
  def __init__(self,**kwargs):
      # Call super class init first
      if (kwargs.get("default_name") == None):
          kwargs["default_name"] = "logistic_regression"
      if (kwargs.get("model_type") == None):
          kwargs["model_type"]=sklearn_utils.logistic_regression_model
  ...
  def loadFromJSON(json_data):
      ...
      if not (jsontype == "hybrid_sklearn.logisticregression_worker.logisticregression_worker"):
          return None

      return logisticregression_worker(**json_data)
  ...
  def update_model(self):
    ...
    model_data = hybrid.data_blob.create ("lr_model_data")
  ...
  def process_observations_core(self, observations, **kwargs):
    ...
    results = model.getMetaData ("lr_results")
    probs = model.getMetaData ("lr_probs")
    classes = model.getMetaData ("lr_classes")

    for observation_index in range (len (observations)):
      self.setMetaData (observations[observation_index], "lr_classes", classes.tolist())
      self.setMetaData (observations[observation_index], "lr_probability", probs[observation_index].tolist())
      self.setMetaData (observations[observation_index], "lr_class", results[observation_index])
