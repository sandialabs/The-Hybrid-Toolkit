...
class Crawler(object):
    def __init__(self, root, depth_limit, confine=None, exclude=[], locked=True, filter_seen=True):
        ...
        self.callback = None  # Callback function for web page visit
    ...
    def _same_host(self, url):
        ...
        try:
            ...
            print >> sys.stderr, "ERROR: Can't parse url '%s' (%s)" % (url, e)
            return False

    def setCallbackOnVisit(self, fun):
        self.callback = fun
    ...
    def crawl(self):
        ...
        while not q.empty():
            ...
            # Just a friendly wait so we don't swamp a website
            # time.sleep(1)
            ...
            if [] == do_not_follow:
                try:
                    ...
                    self.num_followed += 1
                    page = Fetcher(this_url, self.callback)
                    page.fetch()
    ...
    def fetch(self):
        ...
        if handle:
            try:
                ...
                # General processing of webpage
                content = unicode(raw_data, "utf-8", errors="replace")
            ...
            # If there is a callback function registered, call it now
            if self.callback and raw_data:
                self.callback(self, raw_data)
